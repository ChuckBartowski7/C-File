...
void sostituisciilprimo(stringa nome_file,stringa colorevecchio,stringa colorenuovo);
void sostituiscitutti(stringa nome_file,stringa colorevecchio,stringa colorenuovo);
void creaFilePiccoli(stringa nome_file);
void ringraziamenti();
...
int main() {
    ...
    stringa nome_file;
    stpcpy(nome_file,"/Users/chuckbartowski/desktop/colors");
    sostituiscitutti(nome_file,"giallo","accaio");
    //sostituisciilprimo(nome_file,"giallo","accaio");
    ...
    //ringraziamenti();
    //stringa nome_file;
    //stpcpy(nome_file,"/Users/chuckbartowski/desktop/voti");
    //creaFilePiccoli(nome_file);
...
void sostituiscitutti(stringa nome_file,stringa colorevecchio,stringa colorenuovo)
{
    ...
    stringa nome,s1,s2,s3;
    int status;
    int codc1,codc2,codc3;
    long pos;
    for(;;)
    {
        codc1=0;
        ...
        codc3=0;
        pos=ftell(fp);
        status=fscanf(fp,"%s %s %s %s\n",nome,s1,s2,s3);
        if(status==EOF)
            break;
        if(strcmp(s1,colorevecchio)==0)
        {
            codc1=1;
        }
        if(strcmp(s2,colorevecchio)==0)
        {
            codc2=1;
        }
        if(strcmp(s3,colorevecchio)==0)
        {
            codc3=1;
        }
        if((codc1==1)&&(codc2==1)&&(codc3==1))
        {
            fseek(fp,pos,SEEK_SET);
            fprintf(fp,"%s %s %s %s\n",nome,colorenuovo,colorenuovo,colorenuovo);
        }
        else
        if((codc1==1)&&(codc2==1))
        {
            fseek(fp,pos,SEEK_SET);
            fprintf(fp,"%s %s %s %s\n",nome,colorenuovo,colorenuovo,s3);
        ...
        {
          fseek(fp,pos,SEEK_SET);
          fprintf(fp,"%s %s %s %s\n",nome,colorenuovo,s2,colorenuovo);
        }
        else
        if((codc2==1)&&(codc3==1))
        {
          fseek(fp,pos,SEEK_SET);
          fprintf(fp,"%s %s %s %s\n",nome,s1,colorenuovo,colorenuovo);
        }
        else
        if(codc1==1)
        {
            fseek(fp,pos,SEEK_SET);
            fprintf(fp,"%s %s %s %s\n",nome,colorenuovo,s2,s3);
        }
        else
        if(codc2==1)
        {
          fseek(fp,pos,SEEK_SET);
          fprintf(fp,"%s %s %s %s\n",nome,s1,colorenuovo,s3);
        }
        else
        if(codc3==1)
        {
          fseek(fp,pos,SEEK_SET);
          fprintf(fp,"%s %s %s %s\n",nome,s1,s2,colorenuovo);
        }
     }
...
void sostituisciilprimo(stringa nome_file,stringa colorevecchio,stringa colorenuovo)
{
    ...
    {
        pos=ftell(fp);
        stat=fscanf(fp,"%s %s %s %s\n",nome,c1,c2,c3);
        if(stat==EOF)
          break;
        if(strcmp(c1,colorevecchio)==0)
        {
            fseek(fp,pos,SEEK_SET);
            fprintf(fp,"%s %s %s %s\n",nome,colorenuovo,c2,c3);
            cont++;
            verif=1;
        }
        if(strcmp(c2,colorevecchio)==0)
        {
            fseek(fp,pos,SEEK_SET);
            fprintf(fp,"%s %s %s %s\n",nome,c1,colorenuovo,c3);
            cont++;
            verif=1;
        }
        if(strcmp(c3,colorevecchio)==0)
        {
            fseek(fp,pos,SEEK_SET);
            fprintf(fp,"%s %s %s %s\n",nome,c1,c2,colorenuovo);
            cont++;
            verif=1;
        }
    }
...
void creaFilePiccoli(stringa nome_file)
{
    ...
    for(;;)
    {
        ...
        if(status==EOF)
            break;
        stringa nome_fil;
        stpcpy(nome_fil,nome_file);
        strcat(nome_fil,"-");
        strcat(nome_fil,voti[voto]);
        aggiungistudente(nome_fil, nome, voto);
    }
    fclose(fp);
}
//OK!
